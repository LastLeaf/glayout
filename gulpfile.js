...
gulp.task('generate-lib-interfaces-debug', ['compile-lib-debug'], function(cb) {
  fs.writeFile('./lib/bin/interfaces-debug.js', generateLibInterfaces('./lib/bin/glayout-lib.js'), cb)
})
...
gulp.task('generate-lib-interfaces-release', ['compile-lib-release'], function(cb) {
  fs.writeFile('./lib/bin/interfaces-release.js', generateLibInterfaces('./lib/bin/glayout-lib.min.js'), cb)
})
...
gulp.task('build-release', ['compile-asm-release'], function(cb) {
  return gulp.src(['./lib/bin/glayout-lib.min.js', './asm/target/asmjs-unknown-emscripten/release/glayout-main.js'])
    .pipe(concat('glayout.min.js'))
    .pipe(gulp.dest('./bin/'))
})

gulp.task('build-debug-with-sourcemap', ['compile-asm-debug'], function(cb) {
  return gulp.src(['./lib/bin/glayout-lib.js', './asm/target/asmjs-unknown-emscripten/debug/deps/glayout_main-*.js'])
